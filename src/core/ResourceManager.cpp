	...
	std::vector<Geometric*>& geometrics, unsigned int flags)
{
	...
	if(flags & LOAD_NORMALS) aFlags |= aiProcess_GenSmoothNormals;
	if(flags & LOAD_TEXTURES) aFlags |= aiProcess_FlipUVs;
	if(flags & LOAD_TANGENTS) aFlags |= aiProcess_CalcTangentSpace;
...
Geometric* ResourceManager::loadGeometric( const aiMesh* aMesh, int flags)
{
	...
	if(flags & LOAD_NORMALS) dataSize += 3;
	if(flags & LOAD_TEXTURES) dataSize += 2;
	if(flags & LOAD_TANGENTS) dataSize += 3;
	...
	for(unsigned int i=0; i < aMesh->mNumVertices; i++)
	{
		...
		const aiVector3D* aTangent  = aMesh->HasTextureCoords(0) ?
			&(aMesh->mTangents[i]) : &zero3D;
		const aiVector3D* aTexCoord = aMesh->HasTextureCoords(0) ?
			&(aMesh->mTextureCoords[0][i]) : &zero3D;

		unsigned int offset = 0;
		
		verticesData[offset + i * dataSize] = aPos->x; offset++;
		verticesData[offset + i * dataSize] = aPos->y; offset++;
		verticesData[offset + i * dataSize] = aPos->z; offset++;

		if(flags & LOAD_NORMALS)
		{
			verticesData[offset + i * dataSize] = aNormal->x; offset++;
			verticesData[offset + i * dataSize] = aNormal->y; offset++;
			verticesData[offset + i * dataSize] = aNormal->z; offset++;				
		}
		if(flags & LOAD_TEXTURES)
		{
			verticesData[offset + i * dataSize] = aTexCoord->x; offset++;
			verticesData[offset + i * dataSize] = aTexCoord->y; offset++;
		}
		if(flags & LOAD_TANGENTS)
		{
			verticesData[offset + i * dataSize] = aTangent->x; offset++;
			verticesData[offset + i * dataSize] = aTangent->y; offset++;
			verticesData[offset + i * dataSize] = aTangent->z; offset++;
		}
	}
